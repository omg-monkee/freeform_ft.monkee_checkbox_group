...
class Monkee_checkbox_group_freeform_ft extends Freeform_base_ft {
	public 	$info 	= array(
		'name' 			=> 'Monk-ee Checkbox Group',
		'version' 		=> '3.0.0', //Modified Freeform Checkbox Group 5.1.0
		'description' 		=> 'Modified version of the default Checkbox Group fieldtype.'
	);
	...
	public function __construct () {
		...
		$this->info['name'] 		= lang('monkcheck_default_field_name');
		$this->info['description'] 	= lang('monkcheck_default_field_desc');
	}
	//END __construct
	...
	public function display_settings ($data = array())
	{
		...
		ee()->table->add_row(
			lang('monkcheck_default_value', 'default_value') .
				'<div class="subtext">' .
					lang('monkcheck_default_value_desc') .
				'</div>',
			form_input(array(
				'name'		=> 'default_value',
				'id'		=> 'default_value',
		...
		ee()->table->add_row(
			lang('monkcheck_enable_autofocus', 'enable_autofocus') .
			'<div class="subtext">' .
				lang('monkcheck_enable_autofocus_desc') .
			'</div>',
			...
			'&nbsp;&nbsp;' .
			lang('monkcheck_enable', 'enable_autofocus')
		);
		...
		$output .= NBS . NBS . lang('monkcheck_field_none', 'field_none') . NBS . NBS . NBS . NBS . "\n";
		...
		$output .= NBS . NBS . lang('monkcheck_field_label', 'field_label') . NBS . NBS . NBS . NBS . "\n";
		...
		$output .= NBS . NBS . lang('monkcheck_field_desc', 'field_desc') . NBS . NBS . NBS . NBS . "\n";
		...
		$output .= NBS . NBS . lang('monkcheck_field_custom', 'field_custom') . NBS . NBS . NBS . NBS . "\n";
		$output .= "<input type='text' name='field_title_custom' id='field_title_custom' placeholder='" . lang('monkcheck_field_title_custom') . "' value='$title_custom_value'";
		if ($title_value != 'field_custom') { $output .= " disabled"; }
		$output .= " />\n";
		
		ee()->table->add_row(
			lang('monkcheck_field_title', 'field_title') .
				'<div class="subtext">' .
					lang('monkcheck_field_title_desc') .
				'</div>',
			$output
		);
		...
		ee()->table->add_row(
			lang('monkcheck_css_classes', 'css_classes') .
			'<div class="subtext">' .
				lang('monkcheck_css_classes_desc') .
			'</div>',
			...
			'&nbsp;&nbsp;' .
			lang('monkcheck_css_name', 'css_name') .
			'&nbsp;&nbsp;&nbsp;&nbsp;' .
			...
			'&nbsp;&nbsp;' .
			lang('monkcheck_css_type', 'css_type') .
			'&nbsp;&nbsp;&nbsp;&nbsp;' .
			form_input(array(
				'name'		=> 'css_custom',
				'id'		=> 'css_custom',
				'value'		=> $css_custom,
				'placeholder'	=> lang('monkcheck_css_custom'),
				'maxlength'	=> '250',
				'size'		=> '50',
			))
		);
		...
		$text_field = array(
			...
			'id'		=> 'js_action',
			'value'		=> $js_action,
			'placeholder'	=> lang('monkcheck_js_action'),
			'maxlength'	=> '250',
			'size'		=> '50'
		);
		...
		ee()->table->add_row(
			lang('monkcheck_js_event', 'js_event') .
				'<div class="subtext">' .
					lang('monkcheck_js_event_desc') .
				'</div>',
			...
			'&nbsp;&nbsp;' .
			lang('monkcheck_js_confirm', 'js_confirm') .
			'<br /><br />' .
			$output .
			'&nbsp;&nbsp;&nbsp;&nbsp;' .
	...
	public function display_field ($data = '', $params = array(), $attr = array())
	{
		...
		if ($data == '' and !isset($params['default_value'])) {
			$data = explode(",", $this->settings['default_value']);
			foreach ($data as $key => $value) {
				$data[$key] = trim($value);
			}	
		} else if ($data == '' and isset($params['default_value'])) {
			$data = explode(",", $params['default_value']);
			foreach ($data as $key => $value) {
				$data[$key] = trim($value);
			}
		...
		if ($this->settings['css_name'] == "true" or $this->settings['css_type'] == "true" or $this->settings['css_custom'] != '') {
			...
			if ($this->settings['css_type'] == "true") {
				...
				$class .= lang('monkcheck_default_field_type'); }
		...
		foreach ($list_items as $value => $label)
		{
			...
			if ($this->settings['enable_autofocus'] == 'true' and $count == 2) {
...
//END class Monkee_checkbox_group_freeform_ft
